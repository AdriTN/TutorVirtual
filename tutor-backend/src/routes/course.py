...
def _course_to_dict(course: Course):
    return {
        ...
        "subjects": [
            {"id": s.id, "name": s.name, "code": s.description} for s in course.subjects
        ],
    }
...
@router.post("/course/{course_id}/enroll", status_code=204,
...
@router.delete("/course/{course_id}/unenroll", status_code=204,
               dependencies=[Depends(jwt_required)])
