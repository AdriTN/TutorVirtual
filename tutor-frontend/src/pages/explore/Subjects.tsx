import React, { useCallback, useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import NavBar from "../utils/NavBar/NavBar";
import Footer from "../utils/Footer/Footer";
import styles from "./explore.module.css";
import { Course, fetchCourse } from "../../utils/enrollment";
import StepIndicator from "../../components/stepIndicator/StepIndicator";

const SubjectsPage: React.FC = () => {
  /* ---------- hooks ---------- */
  const { id } = useParams<{ id: string }>();   // id del curso
  const nav = useNavigate();

  const [course, setCourse] = useState<Course | null>(null);
  const [q, setQ] = useState("");

  /* ---------- load course ---------- */
  const loadCourse = useCallback(() => {
    if (!id) return;
    fetchCourse(Number(id)).then(setCourse).catch(console.error);
  }, [id]);

  useEffect(loadCourse, [loadCourse]);

  if (!course) return null; // spinner opcional

  /* ---------- search filter ---------- */
  const filtered = course.subjects.filter((s) =>
    s.name.toLowerCase().includes(q.toLowerCase())
  );

  const handleSelect = (subjectId: number) =>
    nav(`/courses/${course.id}/confirm?subject=${subjectId}`);

  /* ---------- UI ---------- */
  return (
    <div className={styles.page}>
      <NavBar />
      <main className={styles.main}>
        {/* Paso indicador (centrado) */}
        <div className={styles.wrapper}>
          <StepIndicator current={2} steps={["CURSO", "ASIGNATURA", "CONFIRMAR"]} />
        </div>

        {/* Título y buscador alineados a la izquierda */}
        <h2 className={styles.left}>{course.title}</h2>
        <div className={styles.searchContainer}>
          <input
            className={styles.search}
            placeholder="Buscar asignatura…"
            value={q}
            onChange={(e) => setQ(e.target.value)}
          />
        </div>

        {/* Grid pantalla completa */}
        <ul className={styles.gridFull}>
          {filtered.map((s) => (
            <li key={s.id} className={styles.card}>
              <h3 className={styles.title}>{s.name}</h3>
              <p className={styles.desc}>{s.description ?? "Sin descripción"}</p>

              {s.enrolled ? (
                <span className={styles.feedback}>Matriculada</span>
              ) : (
                <button
                  className={`${styles.btn} ${styles.primary}`}
                  onClick={() => handleSelect(s.id)}
                >
                  Elegir
                </button>
              )}
            </li>
          ))}
        </ul>
      </main>

      <button
          className={styles.backBtn}
          onClick={() => nav("/courses")}
          aria-label="Volver a cursos"
        >
          <svg
            width="16"
            height="16"
            viewBox="0 0 1024 1024"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M874.7 495.5c0 11.3-9.17 20.47-20.47 20.47H249.4l188.1 188.08c7.99 7.99 7.99 20.95 0 28.94-4 3.99-9.24 5.99-14.47 5.99-5.24 0-10.48-1.99-14.48-5.99L185.4 509.45c-3.84-3.84-5.99-9.05-5.99-14.47s2.15-10.63 5.99-14.47l223.02-223.03c7.99-7.99 20.96-7.99 28.95 0 7.99 8 7.99 20.95 0 28.95L249.4 474.6h604.8c11.3 0 20.47 9.16 20.47 20.9z"/>
          </svg>
          <span>Back</span>
        </button>

      <Footer />
    </div>
  );
};

export default SubjectsPage;
